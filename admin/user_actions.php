...
try {
    // 🚩 IMPORTANT: Ensure 'profile_image' is selected from the database
    $stmt = $pdo->prepare("
...
// Using 'identicon' as the default image to guarantee a picture always appears.
...
function get_status_badge_class($status) {
    ...
    return match($status) {
        'delivered', 'completed' => 'bg-success text-white', // Green
        'preparing', 'shipped' => 'bg-info text-white',      // Blue (for Preparing/Shipped)
        'pending' => 'bg-warning text-dark',                // Yellow (with dark text for visibility)
...
// The final image source for display
        ...
        <div class="col-lg-5">
            <div class="card border-0 shadow-sm h-100">
                ...
                <div class="card-body text-center">
                    ...
                    <p class="mb-2 text-start"><strong>Email:</strong> <a href="mailto:<?= htmlspecialchars($user['email']); ?>"><?= htmlspecialchars($user['email']); ?></a></p>
        ...
        <div class="col-12">
            <div class="card border-0 shadow-sm">
                <div class="card-header bg-secondary text-white">
                    <h5 class="mb-0"><i class="fas fa-cogs me-2"></i> Other Account Actions</h5>
                </div>
                <div class="card-body">
                    <div class="row row-cols-2 row-cols-md-5 g-3 text-center">
                        <div class="col">
                            <a href="mailto:<?= htmlspecialchars($user['email']); ?>" class="btn btn-outline-primary w-100 py-3">
                                <i class="fas fa-envelope fa-2x d-block mb-1"></i>
                                Email
                            </a>
                        </div>
                        <div class="col">
                            <a href="message.php?user_id=<?= $user['id']; ?>" class="btn btn-outline-info w-100 py-3">
                                <i class="fas fa-comment-dots fa-2x d-block mb-1"></i>
                                Message
                            </a>
                        </div>
                        <div class="col">
                            <a href="view_info.php?user_id=<?= $user['id']; ?>" class="btn btn-outline-dark w-100 py-3">
                                <i class="fas fa-user-lock fa-2x d-block mb-1"></i>
                                View Info
                            </a>
                        </div>
                        <div class="col">
                            <a href="suspend.php?user_id=<?= $user['id']; ?>" class="btn btn-outline-warning w-100 py-3">
                                <i class="fas fa-hand-paper fa-2x d-block mb-1"></i>
                                Suspend
                            </a>
                        </div>
                        <div class="col">
                            <a href="disable.php?user_id=<?= $user['id']; ?>" class="btn btn-outline-danger w-100 py-3">
...
document.getElementById('roleUpdateForm').addEventListener('submit', function(e) {
    ...
    // AJAX request to edit_role.php (assuming this file exists in the same directory)
    fetch('edit_role.php', {
        method: 'POST',
